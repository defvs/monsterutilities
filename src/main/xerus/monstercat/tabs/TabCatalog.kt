...
class TabCatalog : TableTab() {
	...
	init {
			...
			try {
				...
				val col = when {
					...
					colName.containsAny("time", "length") ->
						SearchableColumn(colName, Type.LENGTH, converter@ { val split = colValue(it).split(":").map { it.toIntOrNull() ?: return@converter null }; LocalTime.of(0, split[0], split[1]) }, colValue::invoke)
		...
		fill(table)
		table.visibleLeafColumns.addListener(ListChangeListener { it.next(); Settings.VISIBLECATALOGCOLUMNS.putMulti(*it.addedSubList.map { it.text }.toTypedArray()) })
	...
	override fun sheetToData(sheet: List<List<String>>) {
		...
		onJFX {
			...
			table.columns.forEach { col ->
				...
				col.minWidth = (avg - deviation).coerceAtLeast((skin.tableHeaderRow.getColumnHeaderFor(col)?.lookup(".label") as? Label)?.textWidth()?.plus(5) ?: 20.0)
