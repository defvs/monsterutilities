	...
	private fun initialize() {
		...
		add(HBox(chooser.textField(), createButton("Select Folders") {
			Stage().run {
				...
				pane.addRow(2, Label("Singles Subfolder").centerText(), TextField().bindText(DOWNLOADDIRSINGLE).placeholder("Subfolder (root if empty)"), CheckBox().bind(ENABLESINGLEDIRVARS).tooltip("Enable variables"))
				pane.addRow(3, Label("Albums Subfolder").centerText(), TextField().bindText(DOWNLOADDIRALBUM).placeholder("Subfolder (root if empty)"), CheckBox().bind(ENABLEALBUMDIRVARS).tooltip("Enable variables"))
				pane.addRow(4, Label("Mixes Subfolder").centerText(), TextField().bindText(DOWNLOADDIRMIXES).placeholder("Subfolder (root if empty)"), CheckBox().bind(ENABLEMIXESDIRVARS).tooltip("Enable variables"))
				pane.addRow(5, Label("Podcast Subfolder").centerText(), TextField().bindText(DOWNLOADDIRPODCAST).placeholder("Subfolder (root if empty)"), CheckBox().bind(ENABLEPODCASTDIRVARS).tooltip("Enable variables"))
